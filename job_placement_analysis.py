...
figure = px.parallel_categories(filtered_df[[titles[0], titles[-1]]])
st.write(figure)
crosstab_result = pd.crosstab(filtered_df[titles[0]], filtered_df[titles[-1]])
st.write(crosstab_result)

"""#Visualize stream of each Placement Status"""
st.subheader('Stream for Placement Status')
# fig = px.parallel_categories(filtered_df[[titles[2], titles[-1]]])
# st.write(fig)
# crosstab_result_1 = pd.crosstab(filtered_df[titles[2]], filtered_df[titles[-1]])
# st.write(crosstab_result_1)
res = filtered_df.groupby(['stream','placement_status'])['placement_status'].size().reset_index(name='placement_size')
agegp = px.bar(data_frame=res, x='stream', y='placement_size', color='placement_status', title='Placement Status by College').update_layout(xaxis_title='stream', yaxis_title='placement size')
st.plotly_chart(agegp)

"""#Visualize college of each Placement Status"""
st.subheader('College for Placement Status')
res1 = filtered_df.groupby(['college_name','placement_status'])['placement_status'].size().reset_index(name='placement_size')
# st.write(res1)
...
res2 = filtered_df.groupby(['age','placement_status'])['placement_status'].size().reset_index(name='placement_size')
...
res3 = filtered_df.groupby(['years_of_experience','placement_status'])['placement_status'].size().reset_index(name='placement_size')
...
res4 = filtered_df.groupby(['gpa','placement_status'])['placement_status'].size().reset_index(name='placement_size')
...
temp_df = filtered_df[filtered_df["placement_status"] == "Placed"].copy()
...
for i in ["gender", "stream", "placement_status"]:
    filtered_df[i] = le.fit_transform(filtered_df[i])

st.subheader('Correlation Matrix')
#Create a correlation matrix to show relationship between select variables
corr_matrix = filtered_df[["gender","age","stream","gpa","years_of_experience","salary","placement_status"]].corr()
corr_matrix
...
features = ["gender", "age", "stream", "gpa", "years_of_experience"]